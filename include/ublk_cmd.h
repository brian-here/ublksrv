 ...
 * Input:  ctrl_cmd.addr points to struct ublk_shmem_buf_reg
...
/*
 * Set by kernel when request pages match a registered shmem buffer.
 * When set, ublksrv_io_desc.addr encodes buffer index + offset.
 */
...
/* Max number of shared memory buffers per device */
